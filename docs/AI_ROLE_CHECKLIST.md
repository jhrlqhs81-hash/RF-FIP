...
| --- | --- | --- | --- | --- |
| Intent routing | PASS_WITH_RISK | 일반 질문과 RF 분석 질문이 분리되는가 | `isRfAnalysisIntent()`가 shared `hasRfAnalysisIntent()`를 사용하고 hardcode consolidation smoke가 일반/RF 입력을 검증 | RF intent term 확장 시 smoke 갱신 필요 |
...
| Missing checklist link | PASS_WITH_RISK | 누락 체크리스트와 signature가 같은 기준으로 관리되는가 | `SignatureConceptRegistry` relation `requires`와 workflow weight가 missingInfo 후보에 연결됨 | next action gate까지 같은 contract로 묶는 확장 검토 |
| Similar case retrieval | PASS_WITH_RISK | 검색은 local score를 우선하고 LLM은 설명을 보강하는가 | Retrieval weight와 concept-aware key/value 비교가 local similarity 후보에 사용됨 | LLM이 ranking을 바꿀 때 근거 id 표시 강화 필요 |
...
| RAG reference | PASS_WITH_RISK | Wiki/RAG가 source of truth를 대체하지 않는가 | `retrievedKnowledgeContext`는 provider-filtered reference-only context로 `chat-reply`, `rca-summary`에만 적용 | 사내 Wiki/Gauss 실제 connector는 schema 확보 전 blocked |
...
현재 구조는 “Local Engine이 근거와 후보를 만들고, LLM이 설명과 우선순위를 보강한다”는 목표에 대체로 맞습니다. 남은 리스크는 외부 provider 가용성(OpenAI 429), Gauss 사내 schema 미확정, 첨부자료 deep analysis 미완, next action gate와 signature relation의 완전한 중앙화 미완입니다.

## Next Improvement Backlog

1. Intent routing smoke를 shared RF rule catalog 기준으로 계속 유지
2. `SignatureConceptRegistry` relation을 next action gate와 RCA 확정 차단 조건까지 확대
